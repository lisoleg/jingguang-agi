        ...
        with self._lock:
            try:
                if self._m78 is None:
                    from M78_HoTTProofEngine import HoTTProofEngine
                    self._m78 = HoTTProofEngine.get_instance()
            except Exception:
                pass
            try:
                if self._m84 is None:
                    from M84_LiuGuanDynamicsGenerator import LiuGuanDynamicsGenerator
                    self._m84 = LiuGuanDynamicsGenerator.get_instance()
            except Exception:
                pass
            try:
                if self._m88 is None:
                    from M88_TypeCheckFirewall import TypeCheckFirewall
